...
const RealtimeSubscriptionTracker: React.FC = () => { useRealtimeSubscriptions(); return null; };
...
const App: React.FC = () => {
  ...
  const [isDesktopSidebarCollapsed, setIsDesktopSidebarCollapsed] = useState(() => typeof window !== 'undefined' ? localStorage.getItem('emam_sidebar_collapsed') === 'true' : false);
  ...
  const remainingSessionsCount = useMemo(() => {
    ...
    let count = 0;
    for (const key of ['masuk', 'duha', 'zuhur', 'ashar', 'pulang']) { const s = todayRec.sessions?.[key]; if (!s || s.status === 'TS' || s.time === '--:--') { if (['duha', 'zuhur', 'ashar'].includes(key) && todayRec.isHaid) continue; count++; } }
    return count;
  ...
  useEffect(() => { setTickerItems([]); }, []);

  const handleNavigate = (view: ViewState) => {
    if (view === currentView) return;
    const isDeveloper = securityContext?.isDeveloper || activeRoles.includes(UserRole.DEVELOPER);
    if (!isDeveloper && useUIStore.getState().lockedFeatures.includes(view)) { toast.error('Halaman ini sedang dalam perbaikan (Maintenance) dan tidak dapat diakses.'); return; }
    React.startTransition(() => { if (currentView !== ViewState.LOGIN) setNavigationHistory((prev) => [...prev, currentView]); setViewKey((prev) => prev + 1); setCurrentView(view); setIsSidebarOpen(false); });
  };

  const handleBack = () => {
    React.startTransition(() => {
      if (navigationHistory.length > 0) { const historyCopy = [...navigationHistory]; const previousView = historyCopy.pop()!; setNavigationHistory(historyCopy); setViewKey((prev) => prev + 1); setCurrentView(previousView); }
      else if (currentView === ViewState.PUBLIC_SERVICES) { setCurrentView(ViewState.LOGIN); setNavigationHistory([]); setViewKey((prev) => prev + 1); }
      else if (currentView !== ViewState.DASHBOARD && currentView !== ViewState.GUEST_DASHBOARD) { const isGuest = activeRoles.includes(UserRole.TAMU) || userRole === UserRole.TAMU; setCurrentView(isGuest ? ViewState.GUEST_DASHBOARD : ViewState.DASHBOARD); setNavigationHistory([]); setViewKey((prev) => prev + 1); }
    });
  };

  const handleLoginSuccess = (role: UserRole) => {
    const normalizedRole = normalizeRoleStr(role);
    if (lockList.includes('auth_login') && normalizedRole !== UserRole.DEVELOPER) { toast.error('Sistem Sedang Maintenance', { description: 'Hanya akun Developer yang dapat mengakses sistem saat ini.' }); void handleLogout(); return; }
    const resolvedTenantId = securityContext?.tenantId || tenantId;
    if (!resolvedTenantId) { toast.error('Tenant context belum tersedia. Login dibatalkan.'); return; }
    React.startTransition(() => {
      setUserData({ uid: user?.uid || 'logged-in', roles: [normalizedRole], displayName: user?.displayName || 'Pengguna', photoURL: user?.photoURL || null, tenantId: resolvedTenantId } as any);
      setNavigationHistory([]);
      handleNavigate(normalizedRole === UserRole.TAMU ? ViewState.GUEST_DASHBOARD : ViewState.DASHBOARD);
    });
  };

  const handleImpersonate = (role: UserRole, name: string, sid?: string) => { setUserData({ roles: [role], displayName: name, assignment: { studentId: sid || null, teacherId: null, classId: null } } as any); toast.success(`Impersonasi sebagai ${role} (${name}) aktif.`); };
  ...
  if (authLoading) return <div className="fixed inset-0 h-screen w-full flex flex-col items-center justify-center bg-[#020617] z-[1000] overflow-hidden"><Loader2 className="w-5 h-5 text-indigo-400 animate-spin" /></div>;

  const renderContent = () => {
    if (user && currentView !== ViewState.LOGIN) {
      const canonicalValidation = IdentityCompletionService.validate(user);
      if (!canonicalValidation.valid) return <Suspense fallback={<div className="fixed inset-0 bg-[#020617] flex items-center justify-center"><ViewLoader /></div>}><IdentityCompletionPage user={user} onCompleted={(updatedUser) => { setUser(updatedUser); handleNavigate(ViewState.DASHBOARD); }} /></Suspense>;
    }
    if (user && accountStatus === 'needs_id_verification' && currentView !== ViewState.LOGIN) return <Suspense fallback={<ViewLoader />}><ReferenceIdEntryModal isOpen={true} onLogout={handleLogout} /></Suspense>;
    if (user && (accountStatus === 'needs_profile_completion' || accountStatus === 'rejected') && currentView !== ViewState.LOGIN) return <Suspense fallback={<ViewLoader />}><OnboardingForm onLogout={handleLogout} isRejected={accountStatus === 'rejected'} adminNote={(user as any)?.adminNote || ''} /></Suspense>;
    if (user && ['pending','pending_profile_approval','pending_account_approval','needs_data_linkage','pending_data_approval'].includes(accountStatus as string) && currentView !== ViewState.LOGIN && currentView !== ViewState.PROFILE) return <Suspense fallback={<ViewLoader />}><WaitingGate onLogout={handleLogout} accountStatus={accountStatus as any} user={user} /></Suspense>;
    ...
    return (
      <MaintenanceGuard><RealtimeSubscriptionTracker /><ProfileCompletionModal /><div className="h-[100dvh] w-full flex flex-col relative overflow-hidden transition-colors duration-500"><div className="flex-1 w-full flex flex-col lg:flex-row relative overflow-hidden bg-[#f8fafc] dark:bg-[#020617] transition-colors duration-500">{!isLoginPage && !isStandaloneWorkspace && <GlobalSidebar currentView={currentView} onNavigate={handleNavigate} onClose={() => setIsSidebarOpen(false)} isCollapsed={isDesktopSidebarCollapsed} />}<main className="flex-1 flex flex-col h-full overflow-hidden relative"><ImpersonationBanner /><div className="flex-1 overflow-hidden flex flex-col relative w-full"><EmergencyAlert userRole={userRole} /><Toaster position="top-center" expand richColors closeButton toastOptions={{ style: { zIndex: 99999 } }} /><Suspense fallback={<ViewLoader />}>{currentView === ViewState.DASHBOARD && <FloatingActionMenu onNavigate={handleNavigate} />}<ChatbotContainer /><AiAgentContainer /><ChatWindowContainer /><PushNotificationPrompt /></Suspense><div className="flex-1 w-full relative flex flex-col overflow-y-auto"><Suspense fallback={<ViewLoader />}><ViewRenderer key={viewKey} currentView={currentView} handleBack={handleBack} handleNavigate={handleNavigate} onOpenSidebar={() => setIsSidebarOpen(true)} handleLogout={handleLogout} isDarkMode={isDarkMode} toggleTheme={toggleTheme} unreadNotifCount={unreadNotifCount} unreadChatCount={unreadChatCount} pendingLetterCount={pendingLetterCount} pendingApprovalCount={pendingApprovalCount} handleLoginSuccess={handleLoginSuccess} handleImpersonate={handleImpersonate} user={user as any} /></Suspense></div>{!isLoginPage && !isStandaloneWorkspace && <BottomNav currentView={currentView} onNavigate={handleNavigate} userRole={userRole} unreadNotifCount={unreadNotifCount} unreadChatCount={unreadChatCount} pendingLetterCount={pendingLetterCount} pendingApprovalCount={pendingApprovalCount} remainingSessionsCount={remainingSessionsCount} />}</div>{autoFixStatus.isFixing && <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-slate-900/60 backdrop-blur-[2px] animate-in fade-in duration-500"><div className="bg-white dark:bg-[#151E32] p-8 rounded-3xl border border-indigo-500/30 shadow-2xl flex flex-col items-center gap-6 max-w-sm text-center"><div className="relative"><div className="absolute inset-0 bg-indigo-500/20 rounded-full animate-ping scale-150"></div><div className="relative w-16 h-16 rounded-full bg-indigo-600 flex items-center justify-center"><ZapIcon className="w-8 h-8 text-white animate-pulse" /></div></div><h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wide">Self-Healing Protocol</h3></div></div>}{!isOnline && <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[1000] bg-orange-600/95 text-white text-[9px] font-bold px-6 py-2 rounded-2xl">bekerja offline - data tersimpan lokal</div>}</main></div></div></MaintenanceGuard>
    );
  };
