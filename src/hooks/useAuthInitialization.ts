 ...
 * Firebase Auth -> users/{uid} -> referenceId -> domain identity document.
 * No default application user is created from Auth claims.
 */
export const useAuthInitialization = () => {
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(async (firebaseUser) => {
      ...
      if (!firebaseUser) {
        setUser(null); clearUserData(); clearProfile(); setAccountStatus(null);
      ...
      try {
        // Firestore users/{uid} is authoritative. syncAuthUser never provisions
        // a user and never derives role/tenant/referenceId from Firebase Auth.
        ...
        // The validated canonical document is now available in Dexie; the observer
        // only consumes it and must not manufacture identity fields.
        unsubUserDoc = watchUserDoc(firebaseUser.uid, async (data) => {
          try {
            ...
            // referenceId is the only canonical domain identity pointer.
            const referenceId = typeof data.referenceId === 'string' && data.referenceId.trim()
              ? data.referenceId.trim() : null;
            if (!referenceId && !isDev) throw new Error('Canonical user has no referenceId');

            const tenantId = data.tenantId || (isDev ? 'global' : null);
            if (!tenantId && !isDev) throw new Error('Canonical user has no tenantId');

            if (role === UserRole.SISWA && !data.studentsId && referenceId) {
              try {
            ...
            const userData = {
              ...
              assignment: { studentId, teacherId, classId: data.classId || null, positionId: data.positionId || role },
              tenantId, status: data.status || 'active', approvalStatus: data.approvalStatus || 'approved',
              version: data.version || 1, schemaVersion: data.schemaVersion || 1,
            };
            ...
            const profilePayload = {
              ...
              role, roles, studentsId: studentId, teachersId: teacherId, tenantId,
              status: userData.status, referenceId,
            };
            ...
            if (referenceId) {
              ...
              setProfile(profilePayload as any); setProfileLoading(false);
            }
