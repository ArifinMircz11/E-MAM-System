...
import { collection, query, orderBy, limit, where } from '@/services/dbGateway';
...
export function useTeacherAttendanceRecords(selectedClass: string, isManagement: boolean) {
  ...
  const fetchData = useCallback(async () => {
    ...
    await safeCall(async () => {
      ...
      if (targetClass !== 'All') localRecords = localRecords.filter((r) => r.className === targetClass || r.kelas === targetClass);
      ...
      const q = targetClass === 'All'
        ? query(collection(db!, 'teacher_attendance'), orderBy('timestamp', 'desc'), limit(150))
        : query(collection(db!, 'teacher_attendance'), where('className', '==', targetClass), limit(150));
      const docsData = await getDocsOptimized<any>(q);
      if (targetClass !== 'All') {
        docsData.sort((a: any, b: any) => {
          const dateA = a.timestamp ? new Date(a.timestamp.seconds ? a.timestamp.seconds * 1000 : a.timestamp) : null;
          const dateB = b.timestamp ? new Date(b.timestamp.seconds ? b.timestamp.seconds * 1000 : b.timestamp) : null;
