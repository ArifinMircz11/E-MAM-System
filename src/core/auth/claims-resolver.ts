 ...
 * Missing tenant/account/role/reference data is rejected.
 * Legacy fields such as idUnik are never promoted to canonical identity.
 */
export class ClaimsResolver {
  ...
  static mapToCanonical(data: any): CanonicalUser {
    ...
    const referenceId = this.requiredString(data?.referenceId, 'referenceId');
    ...
    const organizationType = this.normalizeOrganizationType(data?.organizationType, accountType);
    ...
    const scopes = Array.isArray(data?.scopes) && data.scopes.length > 0
      ? data.scopes.filter((scope: any) => scope && typeof scope.id === 'string' && typeof scope.type === 'string').map((scope: any) => ({ type: scope.type, id: scope.id }))
  ...
  private static requiredString(value: unknown, field: string): string {
    if (typeof value !== 'string' || value.trim() === '') throw new Error(`[CanonicalUser] Missing required field: ${field}`);
    return value.trim();
  }
  ...
  private static normalizeAccountType(value: unknown): CanonicalAccountType {
    ...
    const allowed: CanonicalAccountType[] = ['DEVELOPER', 'KANWIL', 'KEMENAG', 'MADRASAH', 'ADMIN', 'TEACHER', 'STUDENT', 'PARENT', 'STAFF'];
    if (!allowed.includes(normalized as CanonicalAccountType)) throw new Error(`[CanonicalUser] Unsupported accountType: ${value}`);
    return normalized as CanonicalAccountType;
  }

  private static normalizeRoles(rolesValue: unknown, roleValue: unknown): string[] {
    const roles = Array.isArray(rolesValue) ? rolesValue.filter((role): role is string => typeof role === 'string' && role.trim() !== '') : typeof roleValue === 'string' && roleValue.trim() !== '' ? [roleValue] : [];
    if (roles.length === 0) throw new Error('[CanonicalUser] Missing required field: roles/role');
    return [...new Set(roles.map(role => role.trim().toLowerCase()))];
  }

  private static normalizeOrganizationType(value: unknown, accountType: CanonicalAccountType): CanonicalUser['organizationType'] {
    if (typeof value === 'string' && value.trim() !== '') {
      const normalized = value.trim().toUpperCase();
      if (normalized === 'DEVELOPER' || normalized === 'KANWIL' || normalized === 'KEMENAG' || normalized === 'MADRASAH') return normalized;
    }
  ...
  private static normalizeStatus(value: unknown): CanonicalUser['status'] {
    ...
    if (normalized === 'ACTIVE' || normalized === 'INACTIVE' || normalized === 'PENDING' || normalized === 'DELETED') return normalized;
