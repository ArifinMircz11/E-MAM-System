...
import { XCircleIcon, ClockIcon, SparklesIcon, ArrowLeftIcon, Loader2 } from '@/shared/Icons';
...
const GenericView: React.FC<GenericViewProps> = ({ title, onBack, onOpenSidebar, description }) => {
  const [notice, setNotice] = useState<string>('');
  const [loading, setLoading] = useState(true);

  const defaultNotice =
    'mohon maaf dikarenakan fasilitas kami sangat tidak mendukung satu fitur memakan waktu 30 hari kerja dengan waktu kerja 20 jam/hari';

  useEffect(() => {
    const fetchNotice = async () => {
      try {
        const data = await CacheService.getDocument<any>('settings', 'development_notice');
        if (data) {
          setNotice(data.message || defaultNotice);
        } else {
    ...
    >
      <div className="flex flex-col items-center justify-center min-h-[80vh] p-8 text-center animate-in fade-in zoom-in duration-700">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-900/30 rounded-full border border-amber-100 dark:border-amber-800 mb-6">
          ...
          <span className="text-[10px] font-bold text-amber-700 dark:text-amber-400  tracking-[0.2em]">
            404 - Under Development
          </span>
        </div>

        <h2 className="text-2xl font-bold text-slate-900 dark:text-white uppercase tracking-tight mb-6">
        ...
        <div className="max-w-xs mx-auto p-6 bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800 shadow-inner relative">
          ...
          <div className="absolute -top-3 -left-3 w-8 h-8 bg-indigo-600 rounded-full flex items-center justify-center text-white text-[10px] font-bold italic shadow-lg shadow-indigo-500/30">
            !
          </div>
        </div>

        <p className="text-[10px] text-slate-400 mt-8 max-w-[240px] font-medium leading-relaxed uppercase tracking-wide opacity-60">
        ...
        <div className="mt-12 w-full max-w-xs">
          <button
            onClick={onBack}
            className="w-full py-5 bg-indigo-600 text-white rounded-[1.8rem] font-bold text-[10px] uppercase tracking-[0.3em] shadow-2xl shadow-indigo-500/30 active:scale-95 transition-all flex items-center justify-center gap-3 group"
          >
