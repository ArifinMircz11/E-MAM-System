        ...
        <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-indigo-900 to-slate-900 text-white shadow-xl">
          ...
          <div className="relative p-8 z-10 text-center">
            ...
            <h2 className="text-2xl font-bold mb-2 tracking-tight">
              e-Mam System <span className="text-yellow-400">PRO</span>
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 relative overflow-hidden">
            ...
            <div className="flex items-baseline gap-1 mt-2 mb-4">
              <span className="text-3xl font-bold text-slate-800 dark:text-white">Gratis</span>
            </div>
            ...
            <div className="space-y-3 mb-8">
              {['Data Siswa & Guru', 'Presensi QR Code', 'Laporan Dasar', 'Manajemen Kelas'].map(
                (item, i) => (
                  <div
                    key={i}
              ...
              {['Notifikasi WhatsApp', 'AI Unlimited', 'Custom Branding'].map((item, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 text-sm text-slate-400 dark:text-slate-600"
                >
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border-2 border-indigo-500 relative overflow-hidden shadow-lg shadow-indigo-500/10">
            <div className="absolute top-0 right-0 bg-indigo-500 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl  tracking-wider">
              Paling Laris
            </div>
            ...
            <div className="space-y-3 mb-8">
              {[
                ...
                <div
                  key={i}
                  className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200 font-medium"
                >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            {features.map((feat, idx) => (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center text-white shrink-0 ${feat.color} shadow-md`}
                >
                  {feat.title === 'Cloud Backup Harian' ? (
                    <CloudIconLocal className="w-6 h-6" />
                  ) : (
                ...
                <div>
                  <h4 className="font-bold text-slate-800 dark:text-white text-sm flex items-center gap-2">
                    {feat.title}
                    {feat.premium && (
                      <span className="text-[9px] bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 px-1.5 py-0.5 rounded uppercase font-bold">
                        Pro
                      </span>
                    )}
                  </h4>
