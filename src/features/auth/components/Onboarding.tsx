...
const Onboarding: React.FC<OnboardingProps> = ({ onStart }) => {
  ...
  const steps = [
    {
      ...
      description:
        'Selamat datang di ekosistem digital terpadu MAN 1 Hulu Sungai Tengah. Solusi manajemen modern dalam satu genggaman.',
      icon: SparklesIcon,
    ...
    },
    {
      ...
      description:
        'Lakukan presensi mandiri dengan teknologi QR Code yang cepat dan akurat. Pantau riwayat kehadiran Anda secara realtime.',
      icon: QrCodeIcon,
    ...
    },
    {
      ...
      description:
        'Manfaatkan kecerdasan buatan untuk membantu proses belajar mengajar. Personalisasi pendidikan untuk hasil maksimal.',
      icon: RobotIcon,
  ...
  const handleNext = () => {
    if (step < steps.length - 1) {
      ...
      localStorage.setItem('emam_onboarding_done', 'true');
      onStart();
    }
  };
  ...
  return (
    <div className="h-full w-full bg-white dark:bg-[#020617] flex flex-col relative overflow-hidden transition-colors duration-500">
      {/* Background Ambience */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${currentStep.color} opacity-[0.03] transition-colors duration-700`}
      ></div>
      ...
      <div className="relative h-[55%] w-full overflow-hidden animate-in fade-in zoom-in duration-1000">
        ...
        />
        <div
          className={`absolute inset-0 bg-gradient-to-t from-white dark:from-[#020617] via-transparent to-black/10`}
        ></div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-10 text-center pt-16 pb-12">
        <div key={step} className="animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div
            className={`inline-flex p-3 rounded-2xl bg-gradient-to-br ${currentStep.color} text-white shadow-lg mb-6`}
          >
        ...
        <button
          onClick={handleNext}
          className={`w-full max-w-xs py-4 rounded-[2rem] bg-indigo-600 text-white font-bold text-xs uppercase tracking-[0.25em] shadow-xl shadow-indigo-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-3 group`}
        >
