...
import { getEduContent, getAiQueryCount } from '@/services/geminiService';
import {
  RobotIcon,
  CheckCircleIcon,
  SparklesIcon,
    ...
    <div className="space-y-2">
      {lines.map((line, idx) => {
        ...
        if (trimmed.startsWith('###'))
          return (
            <h3 key={idx} className="text-lg font-bold text-slate-800 dark:text-white mt-4">
              {trimmed.replace(/#/g, '')}
            </h3>
          );
        if (trimmed.startsWith('##'))
          return (
            <h2
              key={idx}
              className="text-xl font-bold text-slate-800 dark:text-white mt-5 border-b border-slate-200 dark:border-slate-700 pb-2"
            >
              {trimmed.replace(/#/g, '')}
            </h2>
          );
        ...
        if (/^\d+\./.test(trimmed)) {
          ...
          return (
            <div key={idx} className="flex gap-2.5 ml-1">
              <div className="font-bold text-indigo-600 dark:text-indigo-400 min-w-[1.5rem]">
                {number}
              </div>
              <div className="text-slate-700 dark:text-slate-300 leading-relaxed">
                {parseBold(content)}
...
export const ContentGeneration: React.FC<ContentGenerationProps> = ({ onBack }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (getAiQueryCount() >= 5) {
      toast.error('Batas kuota maksimal 5 pertanyaan AI telah tercapai.');
      setResult(
        'Maaf, Anda telah mencapai batas maksimal 5 pertanyaan AI per sesi ini. Hubungi administrator madrasah untuk meningkatkan kapasitas asisten pintar.',
      );
      setQueryCount(getAiQueryCount());
      return;
    }
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-6 pb-32 max-w-3xl mx-auto w-full">
        ...
        <div className="bg-gradient-to-br from-indigo-600 via-indigo-700 to-violet-800 rounded-[2rem] p-6 mb-8 text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-xl lg:text-2xl font-bold mb-2 leading-tight">
              Buat Konten Pembelajaran <br />
              Dalam Hitungan Detik
            </h1>
            <p className="text-indigo-100 text-sm opacity-90 max-w-md mt-2 leading-relaxed">
              Gunakan kecerdasan buatan untuk membantu menyusun RPP, Kuis, atau Pengumuman sekolah
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {tools.map((tool) => (
            ...
            >
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center ${tool.bg} ${tool.color} transition-colors mb-1`}
              >
                <tool.icon className="w-6 h-6" />
              </div>
              <span
                className={`text-xs font-bold ${activeTool === tool.id ? 'text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
              >
                {tool.label}
              </span>
              {activeTool === tool.id && (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-1 shadow-sm border border-slate-100 dark:border-slate-700 mb-6 transition-all">
          <div className="p-6">
            <h3 className="font-bold text-slate-800 dark:text-white mb-5 flex items-center justify-between text-sm">
              ...
              <span className="text-[10px] font-extrabold px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 uppercase tracking-wide leading-none">
                AI Quota: {Math.max(0, 5 - queryCount)} / 5 Sisa
              </span>
            </h3>

            <div className="space-y-5">
              <div className="group">
                <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-2  tracking-wider group-focus-within:text-indigo-500 transition-colors ml-1">
                  ...
                  placeholder={
                    activeTool === 'quiz'
                      ...
                      : activeTool === 'announcement'
                        ? 'Misal: Libur Awal Puasa'
                        : 'Misal: Sistem Pernapasan Manusia'
                  }
              ...
              <div className="group">
                ...
                <textarea
                  ...
                  placeholder={
                    activeTool === 'announcement'
                      ? 'Misal: Orang Tua Siswa'
                      : activeTool === 'quiz'
                        ? 'Misal: Sulit (HOTS)'
                        : 'Misal: Kelas XI Semester 2'
                  }
            ...
            {queryCount >= 5 && (
              <div className="mt-4 px-4 py-3 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30 rounded-2xl text-[10px] text-rose-600 dark:text-rose-400 font-extrabold uppercase tracking-wide text-center leading-relaxed">
                ⚠️ Limit 5 Operasi AI Terpenuhi • Silakan hubungi admin sekolah
              </div>
            )}

            <button
              onClick={handleGenerate}
              disabled={loading || !topic || queryCount >= 5}
            ...
            <div className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700 p-4 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {copied ? (
                      <CheckCircleIcon className="w-4 h-4 animate-checkmark" />
                    ) : (
