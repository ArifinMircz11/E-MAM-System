...
const MutationData: React.FC<{ onBack: () => void; userRole: UserRole }> = ({
  onBack,
  userRole,
}) => {
  const { mutasi, loading, fetchMutasi } = useMutasi();
  ...
  return (
    <Layout
      title="Database Mutasi"
      subtitle="Arsip Perpindahan Siswa"
      icon={BriefcaseIcon}
      onBack={onBack}
    >
      <div className="p-4 lg:p-6 pb-40 space-y-6">
        <div className="bg-white dark:bg-[#151E32] p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <input
            type="text"
            placeholder="Cari nama siswa mutasi..."
            value={filterNama}
