 ...
 * Core Enums and Constants to break circular dependencies
 */
...
// 1. PENYELARASAN ENUM ROLE: Wajib lowercase murni untuk validasi frontend
...
export const ROLE_GROUPS: Record<string, UserRole[]> = {
  ...
  OFFICE_STAFF: [UserRole.KEPALA_TU, UserRole.STAF, UserRole.ADMIN],
  STUDENT_FAMILY: [UserRole.SISWA, UserRole.ORANG_TUA, UserRole.KETUA_KELAS],
  ADMIN_LEVEL: [UserRole.DEVELOPER, UserRole.ADMIN],
  ALL_GTK: [
    UserRole.DEVELOPER,
    UserRole.ADMIN,
    UserRole.KEPALA_MADRASAH,
...
export const ADMIN_DEV_ONLY = [UserRole.ADMIN, UserRole.DEVELOPER];
...
export const getAllAuthenticated = (isDeveloper: boolean) => {
  ...
  if (isDeveloper && !list.includes(UserRole.DEVELOPER)) {
    list.push(UserRole.DEVELOPER);
  }
  return list;
};
...
export enum ViewState {
  ...
  ORGANIZATION_MGMT = 'organization_mgmt',
  
  // Developer Console Modules
  DEV_DASHBOARD = 'dev_dashboard',
  DEV_BROADCAST = 'dev_broadcast',
  DEV_FEATURES = 'dev_features',
  DEV_INSTITUTIONS = 'dev_institutions',
  DEV_WORK_UNITS = 'dev_work_units',
  DEV_EDUCATION_LEVELS = 'dev_education_levels',
  DEV_MADRASAH = 'dev_madrasah',
  DEV_USERS = 'dev_users',
  ...
  DEV_SCHEMA_ENGINE = 'dev_schema_engine',
  DEV_INTEGRATION_TEST = 'dev_integration_test',
  DEV_ATTENDANCE_CONTROL = 'dev_attendance_control',
  DEV_DUMMY_ENGINE = 'dev_dummy_engine',
  DEV_POINT_ENGINE = 'dev_point_engine',
  DEV_FIRESTORE_GOV = 'dev_firestore_gov',
  DEV_ARCHITECTURE = 'dev_architecture',
  DEV_GRID = 'dev_grid',
}
