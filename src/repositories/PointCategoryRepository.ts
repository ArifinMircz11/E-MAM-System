...
export class PointCategoryRepository extends BaseRepository<PointCategory> {
  constructor() {
    super('point_categories');
  }

  async findById(id: string, tenantId: string): Promise<PointCategory | null> {
    return (await this.table.where('id').equals(id).filter(c => c.tenantId === tenantId).first()) || null;
  }

  async findAll(tenantId: string): Promise<PointCategory[]> {
    return await this.table.where('tenantId').equals(tenantId).toArray();
  }

  async create(entity: PointCategory): Promise<void> {
    const dataToSave = {
      ...entity,
  ...
  async update(entity: PointCategory): Promise<void> {
    const dataToSave = {
      ...entity,
      syncStatus: 'pending' as any,
      updatedAt: Date.now(),
  ...
  async getActiveByTenant(tenantId: string): Promise<PointCategory[]> {
