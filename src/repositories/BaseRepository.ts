...
export abstract class BaseRepository<T extends BaseEntity> {
  ...
  protected validateContext(context: SecurityContext, operation: string): void {
    if (!context || !context.tenantId || typeof context.tenantId !== 'string' || context.tenantId.trim() === '' || context.tenantId === 'default' || context.tenantId === 'unknown') {
      throw new ArchitectureBoundaryError('repository', 'REPOSITORY_TENANT_MISSING', `Operasi repository '${operation}' ditolak: tenantId tidak valid.`);
    }
  ...
  private assertEntityTenant(context: SecurityContext, entity: Partial<T> | T | undefined, operation: string): void {
    ...
    if (!context.isDeveloper && entityTenantId && entityTenantId !== context.tenantId) {
      throw new ArchitectureBoundaryError('tenant', 'TENANT_ACCESS_DENIED', `Tenant mismatch pada '${operation}': '${entityTenantId}' !== '${context.tenantId}'.`, {
        entityTenantId,
  ...
  async saveBatch(context: SecurityContext, entities: T[] | Partial<T>[]): Promise<T[]> {
    ...
    for (const raw of entities) {
      ...
      if (existing && !context.isDeveloper && (existing as any).tenantId !== context.tenantId) {
        throw new ArchitectureBoundaryError('tenant', 'TENANT_ACCESS_DENIED', `Record '${entity.id}' bukan milik tenant aktif.`);
      }
  ...
  async findById(id: string, tenantId?: string): Promise<T | null> {
    ...
    this.validateContext(context, 'findById');
  ...
  async findAll(tenantId: string): Promise<T[]> {
    const context = getSecurityContext(true);
    this.validateContext(context, 'findAll');
    ...
    return await this.getTable().where('tenantId').equals(context.isDeveloper && tenantId === 'global' ? tenantId : context.tenantId).toArray();
  }
  ...
  async save(context: SecurityContext, entity: Partial<T>): Promise<T> {
    this.assertEntityTenant(context, entity, 'save');
    const table = this.getTable();
    const dbInstance = this.db;
    const tName = this.tableName || table.name || 'students';
    const input = { ...(entity as Partial<T>) } as T & Record<string, any>;
  ...
  async create(...args: [SecurityContext, Partial<T>] | [T]): Promise<T | void> {
    ...
    return this.createWithContext(context, entity);
  }

  private async createWithContext(context: SecurityContext, entity: Partial<T>): Promise<T> {
    this.assertEntityTenant(context, entity, 'create');
    if ((entity as any).id && await this.getTable().get((entity as any).id)) throw new ArchitectureBoundaryError('repository', 'REPOSITORY_CREATE_CONFLICT', `Create ditolak: entity '${(entity as any).id}' sudah ada.`);
  ...
  async delete(context: SecurityContext, id: string): Promise<void> {
    ...
    const tombstone = { ...(existing as any), id, tenantId: context.tenantId, version: nextVersion, syncStatus: SyncStatus.PENDING, deleted: true, deletedAt: now, updatedAt: now };
